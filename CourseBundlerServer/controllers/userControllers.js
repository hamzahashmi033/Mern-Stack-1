...
export const registerUser = catchAsyncError(async(req,res,next)=>{  
    ...
    if(user){
        return next(new ErrorHandler("Email aleardy exists",409))
    }
    const file = req.file
    const fileUri =  getDataUri(file)
    const myCloud = await cloudinary.v2.uploader.upload(fileUri.content,{
        folder:'CourseBundler user profile pic'
    })
   user =  await User.create({
    ...
    sendToken(res,user,"Register user successfully",201)
...
export const loginuser = catchAsyncError(async(req,res,next)=>{  
    ...
    if(!user){
        return next(new ErrorHandler("Credentials are not valid",409))
    }
...
export const forgetPassword = catchAsyncError(async(req,res,next)=>{
    ...
    await sendMail(user.email,"CourseBundler Reset Password",message)
...
export const addToPlaylist = catchAsyncError(async(req,res,next)=>{
    ...
    if(!course) return next(new ErrorHandler("Course Id invalid",404))

    const courseExist = user.playlist.find((item)=>{
        if(item.course.toString() === course._id.toString()) return true
    })
    if(courseExist) return next(new ErrorHandler("Course aleardy exists"),409)
